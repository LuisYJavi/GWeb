...
contract FabricadorClases{
    ...
    function crearContratoTipos(address manager) internal {
        require (msg.sender == fabricaDirClases);
        require(tiposDirContrato == 0);
        tiposDirContrato = new tiposContrato(manager);
        }   
    function crearContratoAlumnos(address manager) internal {
        require (msg.sender == fabricaDirClases);
        require(alumnosDirContrato == 0);
        alumnosDirContrato = new alumnosContrato(manager);
        }   
    
    function crearContratoProfesores(address manager) internal {
        require (msg.sender == fabricaDirClases);
        require(profesoresDirContrato == 0);
        profesoresDirContrato = new profesoresContrato(manager);
        }      
    ...
    function crearNuevaClase (bytes32 _id, uint _fechaHora, uint _maxAlumnos, bytes32 _direccion, address _profesorDir) public {
        ...
        (bytes32 profesorNom, int profesorRat) = P.verProfesorNombreRating(msg.sender);
        require(profesorNom != "");
    ...
    function cierreDeClase(address _claseAlCierre, bytes32 tipoId, uint numAlumnos, address profesorDir, int claseRat) external returns (bool) {
        ...
        (uint claseDur, uint _claseCoste, bool obs) = T.verTipoInfo(tipoId);
...
contract claseContrato {
    ...
    constructor (address _fabrica, address _manager, address _profesorDir,  bytes32 _tipoId, bytes32 _direccion, uint _fecha, uint _maxAlumnos) public {
        ...
        (addT, addA, addP)= F.verDireccionesMaestras();
        ...
        tiposContrato T = tiposContrato(addT);   
        (uint claseDur, uint _claseCoste, bool obs) = T.verTipoInfo(_tipoId);
    ...
    function annadirAlumnos(uint _alumnoId) cNoAcabada cAbierta public {
        ...
        alumnosContrato alumno = alumnosContrato(addA);
        
        (bytes32 alumnoNom) = alumno.annadirAlumnoClase(_alumnoId, address(this), claseCoste);
    ...
    function bajaAlumnos(uint _alumnoId) cNoAcabada cAbierta public {
        ...
        alumnosContrato alumno = alumnosContrato(addA);
        alumno.bajaAlumnoClase(_alumnoId, address(this), claseCoste);
...
contract alumnosContrato {
    ...
    function creaNuevoAlumno(uint id, bytes32 nombre) public returns (bool){
        ...
        alumnos storage alumno = alumnoId[id];
            alumno.alumnoNom = nombre;
            alumno.alumnoRat = 0;
            alumno.alumnoNumClase= 0;
         bool creado = true;
        return(creado);
         }
    ...
    function verAlumnoInfo(uint id) public view returns(bytes32, uint, int, address[]){
        ...
        require (alumnoId[id].alumnoNom != "" );
        alumnos storage alumno = alumnoId[id];
        return(alumno.alumnoNom, alumno.alumnoGyms, alumno.alumnoRat, alumno.alumnoClases);
        } 
    ...
    function annadirAlumnoFondos(uint id, uint numTokens) public returns (bool){
        ...
        require (alumnoId[id].alumnoNom != "" );
        alumnos storage alumno = alumnoId[id];
        alumno.alumnoGyms = SafeMath.add(alumno.alumnoGyms, numTokens);
    ...
    function annadirAlumnoClase(uint _id, address _clase, uint _claseCoste) external returns (bytes32) {
        ...
        alumnos storage alumno = alumnoId[_id];
        require(alumno.alumnoApuntadoClase[_clase] == false);
        require(alumno.alumnoGyms >= _claseCoste);

        alumno.alumnoGyms = SafeMath.sub(alumno.alumnoGyms, _claseCoste);
        alumno.alumnoApuntadoClase[_clase] == true;

        FabricadorClases F = FabricadorClases(fabrica);
        F.ajusteGymsAlPositivo(_claseCoste);             
        return(alumno.alumnoNom);
        }
    ...
    function bajaAlumnoClase(uint id, address _clase, uint _claseCoste) external {
        ...
        alumnos storage alumno = alumnoId[id];
        require(alumno.alumnoApuntadoClase[_clase] == true);
        alumno.alumnoGyms = SafeMath.add(alumno.alumnoGyms,_claseCoste);
        alumno.alumnoApuntadoClase[_clase] == false;
...
contract tiposContrato {
    ...
    function creaNuevoTipo(bytes32 _nombre, uint _dur, uint  _cost ) public {  
        ...
        tipos storage tipo = tipoId[_nombre];
        require (tipo.tipoDuracion == 0);
        tipo.tipoDuracion= _dur;
        tipo.tipoCoste = _cost;
        tipo.tipoObsoleta = false;
